...
GameDisplay::GameDisplay(Game * game, SDL_Rect rect)
   : Display(rect), 
     indicator(rect),
     mplayer_sprite(globals.textures[Cat], globals.tick()*7, 64)
{
   mgame = game;
...
void GameDisplay::update_scores()
{
   ...
   if ( diff != 0 ) {
      mscore_label.update(mgame->player.score());
      indicator.indicate( diff < 0 ? PointGain : PointLoss);
   }
   ...
   if ( diff != 0 ) {
      mhealth_bar.update(mgame->player.health());
      indicator.indicate( diff < 0 ? HpGain : HpLoss);
   }
}
...
void GameDisplay::handle_event(SDL_Event * e)
{
   ...
   switch ( e->key.keysym.sym ) {
      case SDLK_LEFT: case SDLK_a: dir = Left; break;
   ...
   if ( e->type == SDL_KEYDOWN ) active_directions[dir] = true;
   else if ( e->type == SDL_KEYUP ) active_directions[dir] = false;
}
